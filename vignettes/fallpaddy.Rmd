...
The first task is to estimate the predicted difference in net molecular evolution between two lepidosaur species where one of them has undergone one additional net-speciation event^5^. A species' net-molecular divergence is proxied by its phylogenetic path length or root-to-tip distance. A net-speciation event is the cumulative number of branching events (i.e., node count) along a species's root-to-tip path. Net speciation is speciation minus extinction. So, we need to extract the path length and node count from the tree above.
...
It's always good to plot the data. We can understand the data structure, which includes variable distributions, correlations, and outliers or unexpected data points^10^. Data visualization can reveal if our a priori reasoning may not match the pattern in the data. We should not be surprised by our results.

```{r 1_plot_data, out.width="400px"}
plot_data_2d(data = data1, unit = "subs/site")
```

$\\$

The two following steps are necessary, but you need not fully understand them to run downstream analyses.
...
For detailed explanations on the inner workings of PGLS, read Symonds and Blomberg^11^.
...
11. Symonds, M. R. E. & Blomberg, S. P. A primer on phylogenetic generalised least squares. in *Modern Phylogenetic Comparative Methods and Their Application in Evolutionary Biology: Concepts and Practice* (ed. Garamszegi, L. Z.) 105–130 (Springer Berlin Heidelberg, 2014).
